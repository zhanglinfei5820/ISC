...
import com.luckserver.common.FileUtils;
...
public class MyClassLoader extends ClassLoader {
	...
	static {
		...
		jarAbsolutePath = new StringBuilder().append(jarPrefix).append("hibernate-core-4.2.0.Final.jar;")
				.append(jarPrefix).append("hibernate-jpa-2.0-api-1.0.1.Final.jar;").append(jarPrefix)
				.append("validation-api-1.0.0.GA.jar;");
	}

	/**
	 * 如果父的类加载器中都找不到name指定的类， 就会调用这个方法去从磁盘上加载一个类
	 * 
	 * @param name
	...
	public static boolean isClassExist(String name) throws IOException {
		String classPackageName = name.replace(".", File.separator) + ".class";
		return FileUtils.isFileExists(classPath + classPackageName) ? true : false;
	}
	...
	public static void compiler(String name) throws IOException {
		...
		compiler.run(null, null, null, "-encoding", "UTF-8", "-classpath", jarAbsolutePath.toString(),
				javaAbsolutePath);
	}
	...
	public static Class<?> dynamicLoadClass(String name) throws IOException, ClassNotFoundException {
		...
		// if(isJavaExist(name)){
		// if(!FileUtils.deleteFile(classPath +
	...
	public void execute(String jobClassName, String jobClassMethod) {
		try {
			Class<?> dynamicLoadClass = dynamicLoadClass(jobClassName);
			Object newInstance;
			newInstance = dynamicLoadClass.newInstance();
			Method declaredMethod = dynamicLoadClass.getDeclaredMethod(jobClassMethod);
			// 取消了Java的权限控制检查
	...
	public static void main(String[] args)
		...
		// String fileUrl = "com.luckserver.job.ChickenJob";
		// Class<?> dynamicLoadClass = dynamicLoadClass(fileUrl);
		// Object newInstance = dynamicLoadClass.newInstance();
		//// Method declaredMethod = dynamicLoadClass.getDeclaredMethod("execute",
		// int.class,int.class);
		//// Method declaredMethod = dynamicLoadClass.getDeclaredMethod("execute");
		// Method declaredMethod = dynamicLoadClass.getDeclaredMethod("getUser");
		// //取消了Java的权限控制检查
		// declaredMethod.setAccessible(true);
		// Object invoke = declaredMethod.invoke(newInstance);
		// System.out.println(JSON.toJSONString(invoke));
